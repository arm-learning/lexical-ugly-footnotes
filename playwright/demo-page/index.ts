...
export class DemoPage {
    ...
    constructor(page: Page) {
        ...
        this.nestedEditorButton = page.getByRole("button", { name: "Nested Editor" });
    ...
    async addNestedEditor() {
        const initialCount = await this.page.locator('.nested-footnote-demo-wrapper').count();
        await this.mainEditor.focus();
        await this.nestedEditorButton.click();
        // Wait for new nested editor wrapper to appear
        await this.page.waitForFunction((initial) => document.querySelectorAll('.nested-footnote-demo-wrapper').length > initial, initialCount);
        // Also wait for the contenteditable to be present inside the new wrapper
        // We know the new one is the last one
        const lastWrapper = this.page.locator('.nested-footnote-demo-wrapper').last();
    ...
    async typeInNestedEditor(text: string, index = 0) {
        ...
        const nestedEditor = this.page.locator('[contenteditable="true"]').nth(targetIndex);
