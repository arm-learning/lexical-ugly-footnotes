...
function $removeFootnoteById(referenceId: string) {
	...
	for (const { node } of $dfs(root)) {
		...
		if (node instanceof FootnoteReferenceNode && node.getReferenceId() === referenceId) {
			node.remove();
		}
	}
	...
	for (let currentCaret: NodeCaret<"next"> | null = caret; currentCaret !== null; currentCaret = step(currentCaret)) {
