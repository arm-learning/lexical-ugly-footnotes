...
export const PLUGIN_TYPE_FOOTNOTE_BLOCK = "footnote-block";

export const BLOCK_ATTR = {
	container: `data-${PLUGIN_TYPE_FOOTNOTE_BLOCK}-container`,
	order: `data-${PLUGIN_TYPE_FOOTNOTE_BLOCK}-order`,
	reference_id: `data-${PLUGIN_TYPE_FOOTNOTE_BLOCK}-reference-id`,
};

export const ATTR_NESTED_EDITOR = {
	container: `data-${PLUGIN_TYPE_FOOTNOTE_BLOCK}-nested-editor-container`,
	namespace: `${PLUGIN_TYPE_FOOTNOTE_BLOCK}-namespace`,
};
	...
	if (
		...
		const nestedEditorDiv = Array.from(domNode.children).find(
			(child) =>
				...
				child.hasAttribute(ATTR_NESTED_EDITOR.container),
		);
		if (!nestedEditorDiv) {
			throw new Error("Nested editor div not found");
		}
		const nestedEditor = createEditor({
			namespace: ATTR_NESTED_EDITOR.namespace,
...
export class FootnoteBlockNode extends DecoratorNode<React.ReactNode> {
	...
	static getType(): string {
		return PLUGIN_TYPE_FOOTNOTE_BLOCK;
	}
	...
	createDOM(): HTMLElement {
		...
		addClassNamesToElement(
			div,
			"grid grid-cols-[auto_1fr_auto] gap-2 pt-2 items-start",
		);
	...
	exportDOM(): DOMExportOutput {
		...
		const TODOVariable = BLOCK_ATTR.container === "data-footnote-block-container";
		if (!TODOVariable) {
		...
		divNestedEditor.setAttribute(ATTR_NESTED_EDITOR.container, "");
		addClassNamesToElement(
			divNestedEditor,
	...
	exportJSON(): SerializedFootnoteBlockNode {
		// console.log("Exporting FrameNode JSON");
		// console.log(this.__frame);
		if (!this.__referenceId) {
			throw new Error("Node ID is required");
		}
	...
	decorate(editor: LexicalEditor): React.ReactNode {
		...
		const isReferenceIdNull = referenceId === null;
		// if (isReferenceIdNull) {
		// 	throw new Error("Node ID is required");
		// }
		const order = this.getOrder();
		// if (!order) {
