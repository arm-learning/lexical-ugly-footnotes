...
export const DEFAULT_BLOCK_CLASSES: Required<BlockCssClassNames> = {
   ...
   */
  export function getThemeFromConfig(config: EditorConfig): UglyFootnotesTheme | undefined {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-return
    return (config.theme as { uglyFootnotes?: UglyFootnotesTheme })?.uglyFootnotes;
  }
  
  /**
   * Get block classes, merging theme overrides with defaults.
   */
  export function getBlockClasses(config: EditorConfig): Required<BlockCssClassNames> {
   ...
   */
  export function getReferenceClasses(config: EditorConfig): Required<ReferenceCssClassNames> {
    const theme = getThemeFromConfig(config);
   ...
   */
  export function getLineBreakClasses(config: EditorConfig): Required<LineBreakCssClassNames> {
    const theme = getThemeFromConfig(config);
