...
interface HackerNewsPreviewPageProps {
    searchParams: Promise<{ format?: string }>;
}

export default async function HackerNewsPreviewPage({
    searchParams,
}: HackerNewsPreviewPageProps) {
    ...
    if (format === "json") {
        ...
        try {
            ...
            // Note: For Hacker News style, we use default server nodes for preview
            // The HTML output should already be in Hacker News format when saved
            const editor = createHeadlessEditor({
                namespace: "preview-hackernews",
                nodes: [
                    HeadingNode,
                    FootnoteBlockNode,
                    FootnoteReferenceNode,
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-semibold">Hacker News Style Preview</h2>
